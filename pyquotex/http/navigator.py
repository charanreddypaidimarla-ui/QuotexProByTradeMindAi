...
class Browser(Session):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Session pool age tracking — auto-refresh after 30 min
    ...
    def _mount_fresh_adapter(self):
        """Mount a fresh CipherSuiteAdapter to rebuild the connection pool."""
        self.mount(
            'https://',
            CipherSuiteAdapter(
    ...
    def _refresh_pool_if_stale(self):
        """Rebuild connection pool if session is older than max_age.
        This kills stale TCP connections without affecting cookies/token."""
        if time.time() - self._session_created_at > self._session_max_age:
            ...
            try:
                self.close()  # Close all pooled connections
            except Exception:
                pass
            self._mount_fresh_adapter()
            self._session_created_at = time.time()
    ...
    def send_request(self, method, url, headers=None, **kwargs):
        # Auto-refresh stale connection pool
        ...
        # Enforce timeout — prevent infinite hangs on slow/dead connections
