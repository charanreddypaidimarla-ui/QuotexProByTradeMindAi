...
class Quotex:
    ...
    async def get_candles(self, asset, end_from_time, offset, period, progressive=False):
        ...
        # Timeout guard: prevent infinite hang on stalled WebSocket
        _candle_wait_start = time.time()
        _candle_max_wait = 20  # seconds
        while True:
            while self.check_connect and self.api.candles.candles_data is None:
                if time.time() - _candle_wait_start > _candle_max_wait:
                    logger.warning(f"get_candles timeout for {asset} after {_candle_max_wait}s — returning None")
                    return None
    ...
    async def get_result(self, operation_id: str):
        ...
        for item in data_history:
            if item.get("ticket") == operation_id:
                ...
                investment = float(item.get("investmentAmount", 0)) if "investmentAmount" in item else float(item.get("amount", 0))
                if profit > 0 and profit == investment:
                    status = "equal"
                else:
                    status = "win" if profit > investment else "loss"
                return status, item
