...
class WebsocketClient(object):
    ...
    def on_open(self, wss):
        ...
        # ── MAX INTERNET OPTIMIZATION ──
        try:
            raw_sock = wss.sock.sock if hasattr(wss.sock, 'sock') else wss.sock
            if raw_sock:
                raw_sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                raw_sock.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
                raw_sock.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 262144)  # 256KB Rx
                raw_sock.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 262144)  # 256KB Tx
                logger.info("⚡ PyQuotex Socket optimized: TCP_NODELAY + 256KB buffers")
        except Exception as _e:
            logger.debug(f"PyQuotex socket tuning skipped: {_e}")
